...
# Check for sites with no species. If one site has no species this will still allow
# total beta diversity to be calculated but prevent the calculation of components.
# If ore than one site has no species than this will cause NAs for Beta diversity 
# between those sites!
...
# if regional species pools are not being used
...
#  Make sure row names are comids
row.names(mod_com)
row.names(his_com)
...
tree <- ape::keep.tip(tree,colnames(mod_com))

# Functional data prep ---------------------------------------------------------
# removal all species not included in full community data
traits <- traits[row.names(traits) %in% colnames(mod_com),] 

# Final check to ensure all species are in trait data
colnames(mod_com)[!colnames(mod_com) %in% row.names(traits)]
...
trait_hyp <- BAT::hyper.build(trait_gower,axes = 4)  # reduce using pcoa and gowers
...
# 
# ## Data prep ##
# trait_pcs <- as.data.frame(trait_pca$vectors[,1:4]) %>%  # extract the first 4 pcs
#   tibble::rownames_to_column(var = "Sci_name") %>%  # create column for row name
#   left_join(traits %>%
#               tibble::rownames_to_column(var = "Sci_name")) %>%  # merge traits to pc axes by rownames
#   select(Axis.1,Axis.2,Axis.3,Axis.4,MAXBODYL,FECUND,LONG,AGEMAT,
...
#          FASTCURR,MAXTEMP,Rubble,Sand,`Silt/mud`,Vegetation,Various) %>%  # reodrer traits to matach table in appendix
#   rename("PC1" = Axis.1,
#          "PC2" = Axis.2,
#          "PC3" = Axis.3,
#          "PC4" = Axis.4,
...
#   
#   png(filename = paste("trait_pcoa_plots/trait_pcoa_",pc_comb[1,i],"_",pc_comb[2,i],".png",sep = ""),  # create unique file name
#       width = 1600, height = 1600)  # set resolution
#   par(mar=c(8,8,4,2)+1.2)
#   par(mgp=c(6,5,0))
#   plot(trait_pcs[,pc_comb[2,i]] ~ trait_pcs[,pc_comb[1,i]],  # plot every combination of pcs
#        xlab = "",  # remove x label
#        ylab = "",  # remove ylabel
#        pch = 16,  # point type
#        cex = 5,  # point size
...
# 
# ## Corplots
# # trait_cors <- cor(trait_pcs)[5:nrow(cor(trait_pcs)),1:4]  # calcualte pearson correlation between pc axes and traits
# #
# # # Plot correlations
...
alpha_com_list <- parallel::mclapply(seq_len(max_iter),
                                     function(x) taxa_swap_region(his_com,region_bridge),
                                     mc.cores = 10)
names(alpha_com_list) <- sprintf("%03d",seq(1:length(alpha_com_list)))

# Create input lists
# fun_alpha_input <- list(com_list = alpha_com_list, trait = trait_hyp)
phy_alpha_input <- list(time="his",com_list = alpha_com_list, tree = tree)
...
fun_alpha_input <- lapply(com_null_split, 
...
mod_phy_input <- lapply(phy_null_split, 
                        function(x) list(time="mod",com=mod_com,phy_list=x))
his_phy_input <- lapply(phy_null_split, 
...
saveRDS(phy_alpha_input,file.path(phya_dir,"his_phy_alpha_null_input_list.rds"))
purrr::map2(fun_alpha_input,names(fun_alpha_input), 
            function(x,y) saveRDS(x,file.path(funa_dir,paste0("his_fun_alpha_null_input_list_",y,".rds"))))

# Null beta input lists
purrr::map2(fun_input,names(fun_input), 
            function(x,y) saveRDS(x,file.path(fun_dir,paste0("fun_null_input_list_",y,".rds"))))
purrr::map2(phy_input,names(phy_input), 
            function(x,y) saveRDS(x,file.path(phy_dir,paste0("phy_null_input_list_",y,".rds"))))
